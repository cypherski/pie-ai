...
Profile.propTypes = {
  user: PropTypes.shape({
    ...
    discoveries: PropTypes.number.isRequired,
...
const Community = () => {
  ...
  const [users] = useState([]); // Removed setUsers as it's not used

  const tabs = [
    { id: 'feed', name: 'Feed', icon: Icons.Feed },
    { id: 'members', name: 'Members', icon: Icons.Users },
    { id: 'challenges', name: 'Challenges', icon: Icons.Trophy },
  ];
    ...
    setDiscoveries(prevDiscoveries =>
      prevDiscoveries.map(discovery =>
        discovery.id === id
          ? {
              ...discovery,
              hasVoted: !discovery.hasVoted,
              likes: discovery.hasVoted ? discovery.likes - 1 : discovery.likes + 1,
            }
          : discovery,
      ),
    );
